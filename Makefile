isDocker := $(shell sudo docker info > /dev/null 2>&1 && echo 1)
domain := "axprooCRMApp.com"
server := "localhost"
user := $(shell id -u)
group := $(shell id -g)

ifeq ($(isDocker), 1)
	dc := sudo USER_ID=$(user) GROUP_ID=$(group) docker-compose
	dcimport := USER_ID=$(user) GROUP_ID=$(group) docker-compose -f docker-compose.import.yml
	dcimport := USER_ID=$(user) GROUP_ID=$(group) docker-compose -f docker-compose.import.yml
	de := sudo docker-compose exec
	dr := $(dc) run --rm
	cisp := $(de) php php spark
...
sync: ## Récupère les données depuis le serveur
	rsync -avz --ignore-existing --progress --exclude=avatars axproo:/home/axproo/axprooCRMApp.com/public/uploads/ ./public/uploads/
...
install: vendor/autoload.php public/assets/manifest.json ## Installe les différentes dépendances
	...
	make migrate
	APP_ENV=prod APP_DEBUG=0 $(cs) cache:clear
	$(cs) messenger:stop-workers
...
dump: var/dump ## Génère un dump SQL
	$(de) db sh -c 'mysqldump -u adminaxproo -p"adminaxproo" "axprooCRMAppDATABASES" --dump-date --no-autocommit --single-transaction --hex-blob --triggers -R -E  > /var/www/var/dump/dump.sql'

.PHONY: dumpimport
dumpimport: ## Import un dump SQL
	$(de) db sh -c 'mysql -u adminaxproo -p"adminaxproo" axprooCRMAppDATABASES < /var/www/var/dump/dump.sql'

.PHONY: seed
seed: vendor/autoload.php ## Génère des données dans la base de données (docker-compose up doit être lancé)
	$(sy) doctrine:migrations:migrate -q # php vendor/bin/doctrine orm:schema-tool:update --force --dump-sql
